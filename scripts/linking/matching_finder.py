...
class Max_matching_finder:
    def __init__( self):
        self.linear_program_name = "minimum_weighted_bipartite_matching"
        self.objective_sense = pulp.LpMaximize
        self.act_opt_init = 0
    ...
    def find_matching_new( self, a_num, b_num, value_table):
        a_vertices = list(range(a_num))
        b_vertices = list(range(b_num))
        edge_dict = { ( a, b ): value_table[ a ][ b ]
                      for a in a_vertices for b in b_vertices }
        linear_program = pulp.LpProblem( self.linear_program_name,
                                         sense=self.objective_sense)
        chosing_var = pulp.LpVariable.dicts( "chosing_var", edge_dict.keys(), cat="Binary")
        linear_program += pulp.lpSum( edge_dict[ edge ] * chosing_var[ edge ]
                                      for edge in edge_dict)
        for vertex in a_vertices + b_vertices:
            chosen_edges = pulp.lpSum( chosing_var[ edge ] for edge in edge_dict
                                       if vertex in edge)
            linear_program += chosen_edges <= 1
        linear_program.solve( pulp.PULP_CBC_CMD( msg=0))
        opt_pairs_list = [ edge for edge in edge_dict
                             if pulp.value( chosing_var[ edge ]) == 1 ]
        opt_val = linear_program.objective.value()
        return opt_pairs_list, opt_val
...
if __name__ == "__main__":
    ...
    mmf = Max_matching_finder()
    chosen_pairs, sum_opt_val = mmf.find_matching_new( a_num, b_num, value_table)\
    # chosen_pairs, sum_opt_val = greedy_MWB_matching(
    #     a_num, b_num, value_table, True)
    print( round( sum_opt_val, 3))
