...
class Ud_linker( Linker):
    ...
    def compute_frame_pair_score(self, a_frame_inst, b_frame_inst, _, __):
        ...
        return self.weigh_params( feats_points) / 6

    def compute_arg_pair_score( self, a_arg_inst, b_arg_inst, _, __):
        a_arg_upostag = a_arg_inst.upostag
        b_arg_upostag = b_arg_inst.upostag

        score = int( a_arg_upostag == b_arg_upostag)
    ...
    def get_params( self):
        param_string = "UD params:  "
        param_names = {"verb_parent_ord", "verb_parent_upos", "verb_ord"
                       "verb_deprel", "verb_depth", "verb_child_num"}
        for param_name , param_weight in zip(param_names, self.params_weights):
            param_string += param_name + "=" + \
                            str( round( param_weight * 100, 2)) + ", "
        param_string += "threshold=" + str( self.threshold)
        return param_string
