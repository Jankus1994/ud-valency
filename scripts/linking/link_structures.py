...
class Frame_type_arg_link:
    ...
    def get_other_frame_type_arg( self, frame_type_arg):  # -> Frame_type_arg
        """ not used in the general system (called from vallex_evaluator) """
        if frame_type_arg is self.a_frame_type_arg:
            return self.b_frame_type_arg
        elif frame_type_arg is self.b_frame_type_arg:
            return self.a_frame_type_arg
        else:
            return None
...
class Frame_inst_link:
    ...
    def get_other_frame_inst( self, frame_inst):  # -> Frame_inst
        """ not used in the general system (called from vallex_evaluator) """
    ...
    def get_other_frame_inst_by_type( self, frame_type):  # -> Frame_inst
        """ not used in the general system (called from vallex_evaluator) """
        if frame_type is self.a_frame_inst.type:
            return self.b_frame_inst
        elif frame_type is self.b_frame_inst.type:
            return self.a_frame_inst
        else:
            return None
...
class Frame_type_link:
    ...
    def _link_args( self):  # void
        ...
        arg_link_num = 1
        b_free = [ True ] * len( b_frame_type_args)
        for a_frame_type_arg in a_frame_type_args:
            for i, b_frame_type_arg in enumerate( b_frame_type_args):
                if a_frame_type_arg.deprel == b_frame_type_arg.deprel and b_free[ i ]:
